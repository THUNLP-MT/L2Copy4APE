...
def add_timing_signal_by_pos(x, position, min_timescale=1.0, max_timescale=1.0e4, name=None):
    """
    This function adds a bunch of sinusoids of different frequencies to a
    Tensor. See paper: `Attention is all you need'

    :param x: A tensor with shape [batch, length, channels]
    :param position: A tensor with shape [batch, length]
    ...
    with tf.name_scope(name, default_name="add_timing_signal", values=[x, position]):
        length = tf.shape(x)[1]
        ...
        scaled_time = (tf.expand_dims(position, 2) *
                       tf.reshape(inv_timescales, [1, 1, num_timescales]))
...
def multiplicative_attention(queries, keys, values, bias, keep_prob=None,
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if scaled_bias is not None:
            logits -= tf.reduce_min(logits, axis=3, keep_dims=True)
            logits *= tf.expand_dims(tf.expand_dims(scaled_bias, axis=1), axis=1)
