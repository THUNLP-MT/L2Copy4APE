...
def _get_inference_fn(model_fns, features):
    def inference_fn(inputs, state):
        local_features = {
            ...
            "source_pos": features["source_pos"],
            "origin_length": features["origin_length"],
            # [bos_id, ...] => [..., 0]
...
def create_inference_graph(models, features, params):
    ...
    features["source_pos"] = tf.expand_dims(features["source_pos"], 1)
    features["source_pos"] = tf.tile(features["source_pos"], [1, beam_size, 1])
    ...
    features["source_pos"] = tf.reshape(features["source_pos"],
                                    [shape[0] * shape[1], shape[2]])
    ...
    features["source_length"] = tf.tile(features["source_length"],
                                        [1, beam_size])
    features["origin_length"] = tf.expand_dims(features["origin_length"], 1)
    features["origin_length"] = tf.tile(features["origin_length"],
                                        [1, beam_size])
    shape = tf.shape(features["source_length"])

    max_length = features["origin_length"] + decode_length

    # [batch, beam_size, length] => [batch * beam_size, length]
    features["source_length"] = tf.reshape(features["source_length"],
                                           [shape[0] * shape[1]])
    features["origin_length"] = tf.reshape(features["origin_length"],
                                           [shape[0] * shape[1]])
