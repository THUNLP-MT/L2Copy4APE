...
def get_training_input(filenames, params):
    ...
    with tf.device("/cpu:0"):
        ...
        trans_dataset = tf.data.TextLineDataset(filenames[1])
        tgt_dataset = tf.data.TextLineDataset(filenames[2])
        tag_dataset = tf.data.TextLineDataset(filenames[3])

        dataset = tf.data.Dataset.zip((src_dataset, trans_dataset, tgt_dataset, tag_dataset))
        ...
        # Split string
        dataset = dataset.map(
            lambda src, trans, tgt, tag: (
                tf.string_split([src]).values,
                tf.string_split([trans]).values,
                tf.string_split([tgt]).values,
                tf.string_to_number(tf.string_split([tag]).values, tf.int32)
            ),
        ...
        dataset = dataset.map(
            lambda src, trans, tgt, tag: (
                ...
                tf.concat([trans, [tf.constant(params.eos)]], axis=0),
                tf.concat([tgt, [tf.constant(params.eos)]], axis=0),
                tf.concat([tf.to_int32(tf.ones(tf.shape(src)+1)), tag, [tf.constant(1)]], axis=0)
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.map(
            lambda src, trans, tgt, tag: {
                "source": tf.concat([src, trans], axis=0),
                "target": tgt,
                "tagging": tag,
                "source_pos": tf.concat([tf.range(tf.shape(src)[0]), tf.range(tf.shape(trans)[0])], axis=0),
                "source_length": tf.shape(src)+tf.shape(trans),
                "origin_length": tf.shape(src),
...
def sort_input_file(filenames, reverse=True):
    inputs = []
    input_lens = []
    files = [tf.gfile.GFile(name) for name in filenames]
...
def get_evaluation_input(inputs, params):
    with tf.device("/cpu:0"):
        ...
        dataset = dataset.map(
            lambda src, trans, pe, tag: (
                src,
                trans,
                tf.concat([tf.to_int32(tf.ones(tf.shape(src))), tag, [tf.constant(1)]], axis=0),
                pe
            ),
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.map(
            lambda src, trans, tag, *x: {
                "source": tf.concat([src, trans], axis=0),
                "tagging": tag,
                "source_pos": tf.concat([tf.range(tf.shape(src)[0]), tf.range(tf.shape(trans)[0])], axis=0),
                "source_length": tf.shape(src)[0]+tf.shape(trans)[0],
                "origin_length": tf.shape(src)[0],
                "references": x
            },
            num_parallel_calls=params.num_threads
        )
        ...
        dataset = dataset.padded_batch(
            params.eval_batch_size,
            {
                ...
                "tagging": [tf.Dimension(None)],
                "source_pos": [tf.Dimension(None)],
                "source_length": [],
                "origin_length": [],
